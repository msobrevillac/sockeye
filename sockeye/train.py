...
def create_data_iters(args: argparse.Namespace,
    ...
    path_sources = [os.path.abspath(source) for source in args.source]
    path_targets = [os.path.abspath(target) for target in args.target]
    path_source_graphs = [os.path.abspath(source_graph) for source_graph in args.source_graphs]

    logger.info(path_sources)
    logger.info(path_targets)
    logger.info(path_source_graphs)

    return data_io.get_training_data_iters(source=path_sources,
                                           target=path_targets,
                                           source_graphs=path_source_graphs,
